...
int main()
{
	...
	while(1)
	{
		...
		{				
			...
			for(i=0;i<n;i++)							
			{
				...
				for(j=0;j<k;j++)
				{
					...
					// formula: |a1 - b1| + |a2 - b2|
					if(op2 == 2)
						d = fabs(mean[j][0] - pts[i][0]) + abs(mean[j][1] - pts[i][1]);
		...
		ofstream fmout("out_means.csv", ios::out);
		...
		for(i=0;i<k;i++)
		{
			...
			itoa(i,x,10);
			strcat(x,".csv");
	        ofstream fcout(x, ios::out);
			
			printf("%i\t\t%.2f,%.2f\t\t",i+1,mean[i][0],mean[i][1]);
			fmout << mean[i][0] << "," << mean[i][1] << endl; 
			printf("{");
			printf("(%.2f,%.2f)",pts[c[i][0]][0],pts[c[i][0]][1]);
			fcout << pts[c[i][0]][0] << "," << pts[c[i][0]][1] << endl;                                         
			for(j=1;j<=top[i];j++)
			{
				printf(", (%.2f,%.2f)", pts[c[i][j]][0],pts[c[i][j]][1] );
				fcout << pts[c[i][j]][0] << "," << pts[c[i][j]][1] << endl;      
			}
			printf("}\t");
			printf("\n");
		}
